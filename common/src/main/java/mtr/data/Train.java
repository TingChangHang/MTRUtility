...
public abstract class Train extends NameColorDataBase implements IPacket {
			...
			if (!isOnRoute) {
				...
				if (railProgress >= distances.get(distances.size() - 1) - (railLength - trainCars * spacing) / 2) {
					...
					if (speed <= 0) {
						...
						if (!transportMode.continuousMovement && stoppingDistance < 0.5 * speed * speed / accelerationConstant && !isCurrentlyManual) {
					...
					if (!transportMode.continuousMovement && railProgress > distances.get(nextStoppingIndex)&&(!isCurrentlyManual)&&(!isCurrentlyManual)) {
	...
	public static RailType convertMaxManualSpeed(int maxManualSpeed) {
		if (maxManualSpeed >= 0 && maxManualSpeed <= RailType.BEDROCK.ordinal()) {
